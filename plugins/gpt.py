...
async def handle_discord_message(message, interface):
    """
    ...
    if message.content.startswith(cfg.config['discord_prefix'] + 'gpt'):
        ...
        if prompt:
            ...
            # If interface is provided, also send the response over the mesh network
            # if interface:
...
def handle_meshtastic_message(packet, interface, client):
    """
